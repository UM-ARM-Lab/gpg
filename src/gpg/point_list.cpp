...
PointList PointList::slice(const std::vector<int>& indices) const
{
  ...
  Eigen::MatrixXi regions_out = EigenUtils::sliceMatrix(regions_, indices);

  return PointList(points_out, normals_out, cam_source_out, regions_out, view_points_);
}
...
PointList PointList::transformToHandFrame(const Eigen::Vector3d& centroid, const Eigen::Matrix3d& rotation) const
{
  ...
  return PointList(points_centered, normals, cam_source_, regions_, view_points_);
}
...
PointList PointList::rotatePointList(const Eigen::Matrix3d& rotation) const
{
  ...
  return PointList(points, normals, cam_source_, regions_, view_points_);
}
